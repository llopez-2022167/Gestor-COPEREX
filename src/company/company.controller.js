...
//Importacion del documento excel 
...
export const update = async(req, res)=>{
    try{
        let data = req.body;
        let {id}= req.params;
        let updete= checkUpdate(data, id);
        if(!updete) return res.status(400).send(
            {
        ...
        if(!updated) return res.status(401).send(
            {
                message: 'seProduct not found and not update'
            });
        return res.send({message: 'Updated product', updated});
    }catch(err){
        console.error(err);
        return res.status(500).send({message: 'error updeting'});
    }
}
